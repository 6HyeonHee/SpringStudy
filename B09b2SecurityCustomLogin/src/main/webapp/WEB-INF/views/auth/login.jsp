...
<body class="m-5">
	...
	<c:if test="${ empty user_id }" var="loginResult">
		...
		failureUrl 혹은 failureHandler 메서드를 비활성화해둔 경우에만
		...
		<!-- 시큐리티 설정 파일에 지정된 요청명으로 action 속성값 기술 -->
		<form action="/myLoginAction.do" method="post">
			<!-- 아이디와 비밀번호도 설정 파일에 지정된 값으로 속성값을 기술한다. -->
	...
	<!-- 로그인되었을 때 출력되는 부분, 로그인 아이디와 로그아웃 버튼 출력 -->
